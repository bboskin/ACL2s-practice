...
(defunc max-len (i res max)
  :input-contract (and (natp i) (natp max) (<= i max) (resp res))
  :output-contract (resp (max-len i res max))
  (cond ((equal i max) res)
        (t (let ((curr (c-trace-count i 0)))
             (if (> curr (second res))
               (max-len (+ i 1) (list i curr) max)
               (max-len (+ i 1) res max))))))

(defunc loop (res count)
  :input-contract (and (natp count) (resp res))
  :output-contract (resp (loop res count))
  (cond ((>= count 1000000) res)
        (t (let ((new (max-len count (list 0 0) (+ count 10000))))
             (if (> (second new) (second res))
               (loop new (+ count 10000))
               (loop res (+ count 10000)))))))


(defconst *max* (loop (list 0 0) 1))
